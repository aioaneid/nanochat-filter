...
kernel void up_sweep_kernel_shared(
    ...
    // [FIX]: Now identical to Plane (distinguishes in_idx from stack idx)
    ...
    // --- Implementation Divergence Start ---
    // Shared Memory Variant
    threadgroup float shm_a[1024];
    threadgroup float shm_x[1024];

    shm_a[unit_idx] = cur_a;
    shm_x[unit_idx] = cur_x;

    for (uint offset = 1; offset < chunk_size; offset <<= 1) {
        threadgroup_barrier(mem_flags::mem_threadgroup);

        if (unit_idx >= offset) {
            float n_a = shm_a[unit_idx - offset];
            float n_x = shm_x[unit_idx - offset];
...
pub fn ltv_shared_metal(
    ...
    // Actually, the fastest seems to be the value 32, reaching parity with plane.
    let max_threads = metal_ctx
        .up_sweep_pipeline
        .max_total_threads_per_threadgroup() as u32;
    let geometry = create_geometry_from_schedule(
        if schedule == "pmax" { "max" } else { schedule },
        t_seq,
        parallel_a,
        r,
        if schedule == "pmax" {
            max_threads.min(metal_ctx.up_sweep_pipeline.thread_execution_width() as u32)
        } else {
            max_threads
        },
    )
...
pub fn init_ltv_shared_metal_context_for_default_device() -> PyResult<()> {
    let metal_context =
        metal_context_for_default_device(UP_SWEEP_KERNEL_SOURCE, "up_sweep_kernel_shared")?;
    info!(
        "Metal UpSweep pipeline max_total_threads_per_threadgroup: {}",
        metal_context
            .up_sweep_pipeline
...
pub fn release_ltv_shared_metal_context() {}
