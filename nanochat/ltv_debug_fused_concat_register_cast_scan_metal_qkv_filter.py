...
class LtvDebugFusedConcatRegisterCastScanMetalQkvFilter(LtvCombinedQkvFilter):
    def __init__(
        self,
        n_head: int,
        n_kv_head: int,
    ...
    def forward(self, x, previous_values: LtvInitValues):
        ...
        self.alpha_logit_callback(
            logits_slice.view(B, T, self.total_active_heads, self.num_alpha_groups)
        )
        ...
        hq, hk, hv = torch.ops.nanochat.ltv_fused_concat_register_cast_scan_metal_op(
            combined,
            inits,
            self.n_head,
            self.n_kv_head,
