...
import nanochat.ops.ltv_triton_fused_concat_scan  # Force execution of library.impl decorator


logger = logging.getLogger(__name__)


class LtvFusedConcatScanTritonQkvFilter(LtvCombinedQkvFilter):
    def forward(self, x, previous_values: LtvInitValues):
        ...
        with torch.amp.autocast(device_type=x.device.type, enabled=False):
            ...
            hq, hk, hv = torch.ops.nanochat.ltv_fused_concat_scan_triton_op(
                combined_f32,
                logit_bias_f32,
                inits_f32,
                self.n_head,
