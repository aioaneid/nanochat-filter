...
from nanochat import gpt_factory
...
def make_stop_tokens(tokenizer):
    assistant_end = tokenizer.encode_special("<|assistant_end|>")
    bos = tokenizer.get_bos_token_id()
    return (assistant_end, bos)
...
class Engine:
    ...
    def generate(
        ...
        if stop_tokens is None:
            stop_tokens = make_stop_tokens(self.tokenizer)

        # 1) Run a batch 1 prefill of the prompt tokens
        m = self.model.config
        kv_model_kwargs = {
            "num_q_heads": m.n_head,
    ...
    def generate_batch(self, tokens, num_samples=1, *, stop_tokens=None, **kwargs):
        """
        Non-streaming batch generation that returns the final token sequences.
        Terminal tokens (<|assistant_end|>, <bos>) stop a row by default.
        Pass stop_tokens=[] to disable early stopping.
        """
        if stop_tokens is None:
            stop_tokens = make_stop_tokens(self.tokenizer)
        ...
        for token_column, token_masks in self.generate(
            tokens, num_samples, stop_tokens=stop_tokens, **kwargs
        ):
...
if __name__ == "__main__":
    ...
    model, tokenizer, meta = load_model(
        gpt_factory.make_split_original_model, "base", device, phase="eval"
    )
