...
try:
    ...
    if not torch.cuda.is_bf16_supported(including_emulation=False):
        for f in [
            cpp_ewma_cuda.ltv_fused_concat_head_major_register_cast_blelloch_scan_cuda_forward,
            cpp_ewma_cuda.ltv_fused_concat_head_major_register_cast_blelloch_scan_cuda_backward,
        ]:
...
def forward_kernel(
    ...
    return ext.ltv_fused_concat_head_major_register_cast_blelloch_scan_cuda_forward(
        combined,
        inits,
        logit_bias,
        out_q,
...
def forward_op(
    ...
    torch.ops.nanochat_cuda.ltv_fused_concat_head_major_register_cast_blelloch_scan_cuda_forward_kernel(
        combined,
        inits,
        logit_bias,
        out_q,
...
def backward_kernel(
    ...
    ext = _require_cpp_ewma_cuda()
    return ext.ltv_fused_concat_head_major_register_cast_blelloch_scan_cuda_backward(
        combined,
        inits,
        logit_bias,
        hq,
...
def backward(ctx, grad_hq, grad_hk, grad_hv):
    ...
    grad_logit_bias = torch.zeros_like(logit_bias, memory_format=torch.contiguous_format)

    torch.ops.nanochat_cuda.ltv_fused_concat_head_major_register_cast_blelloch_scan_cuda_backward_kernel(
        combined,
        inits,
        logit_bias,
        hq,
