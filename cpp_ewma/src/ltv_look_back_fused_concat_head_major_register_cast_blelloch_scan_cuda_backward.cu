...
ltv_look_back_fused_concat_head_major_register_cast_cub_backward_device(
...
{
  LTV_LOOK_BACK_FUSED_CONCAT_HM_BACKWARD_DEVICE_BODY(
      T_in, T_compute, T_out, kScanThreads, kRThreads, kRItems,
      UseSigmoid, P, Q, K,
      combined, inits, logit_bias, hq, hk, hv, grad_hq, grad_hk, grad_hv,
      grad_combined, grad_inits_accum, grad_logit_bias, grad_logit_accum,
...
__device__ __forceinline__ void
ltv_look_back_fused_concat_head_major_register_cast_cub_backward_device_const(
    const T_in *__restrict__ combined, const T_in *__restrict__ inits,
    const T_compute *__restrict__ logit_bias, const T_out *__restrict__ hq,
    const T_out *__restrict__ hk, const T_out *__restrict__ hv,
    const T_out *__restrict__ grad_hq, const T_out *__restrict__ grad_hk,
...
// 1) Generic backward kernel (unchanged, calls generic device function)
template <typename T_in, typename T_compute, typename T_out,
          int kScanThreads, int kRThreads, int kRItems,
          bool UseSigmoid, int P, int Q, int K>
__global__ void ltv_look_back_fused_concat_cub_backward_kernel(
    const T_in *__restrict__ combined, const T_in *__restrict__ inits,
    const T_compute *__restrict__ logit_bias, const T_out *__restrict__ hq,
    const T_out *__restrict__ hk, const T_out *__restrict__ hv,
    const T_out *__restrict__ grad_hq, const T_out *__restrict__ grad_hk,
...
// 2) Constant‑strides backward kernel (now calls const device function)
template <typename T_in, typename T_compute, typename T_out,
          int kScanThreads, int kRThreads, int kRItems,
          bool UseSigmoid, int P, int Q, int K,
...
__global__ void ltv_look_back_fused_concat_cub_backward_kernel_const(
    const T_in *__restrict__ combined, const T_in *__restrict__ inits,
    const T_compute *__restrict__ logit_bias, const T_out *__restrict__ hq,
    const T_out *__restrict__ hk, const T_out *__restrict__ hv,
    const T_out *__restrict__ grad_hq, const T_out *__restrict__ grad_hk,
...
constexpr bool is_dispatch_valid_backward()
{
  ...
  if (block_size > 1024)
    return false;
  constexpr size_t static_smem = backward_shared_memory_bytes<T_compute, kScanThreads, kRThreads, kRItems>();
...
{
  constexpr bool kValid = is_dispatch_valid_backward<T_compute, kScanThreads, kRThreads, kRItems>();
  if constexpr (kValid)
  {
    int smem_bytes = static_cast<int>(backward_shared_memory_bytes<T_compute, kScanThreads, kRThreads, kRItems>());
         ...
         backward_has_transposed_qkv_grad_output_strides))
    {
      ...
      {
        ltv_look_back_fused_concat_cub_backward_kernel_const<
            T_in, T_compute, T_out, kScanThreads, kRThreads, kRItems, cSig,
      ...
      {
        TORCH_CHECK(false, "We already checked all branches.");
      }
      copy_grad_logits();
      return;
    }
    ...
    ltv_look_back_fused_concat_cub_backward_kernel<
        T_in, T_compute, T_out, kScanThreads, kRThreads, kRItems, cSig,
        cP, cQ, cK>
        <<<grid, block, smem_bytes, stream>>>(
            combined.data_ptr<T_in>(), inits.data_ptr<T_in>(),
  ...
  AT_DISPATCH_SWITCH(
      ...
      AT_DISPATCH_CASE(at::ScalarType::Float, [&]
        ...
        ltv_look_back_backward_dispatch<T_in, T_compute, T_out>(
            ...
            s_gq_b, s_gq_h, s_gq_t, s_gq_d,
            s_gk_b, s_gk_h, s_gk_t, s_gk_d,
            s_gv_b, s_gv_h, s_gv_t, s_gv_d); }) AT_DISPATCH_CASE(at::ScalarType::BFloat16, [&]
                                             {
